...
- **`ghcr.io/lostmaniac/picoclaw-browser`** — full image with Chromium (default/recommended)
- **`ghcr.io/lostmaniac/picoclaw`** — slim image without browser
...
docker build --build-arg INSTALL_BROWSER=true -t picoclaw-browser .

# Build without browser (slim)
docker build --build-arg INSTALL_BROWSER=false -t picoclaw .
...
- **`.github/workflows/docker.yml`** — CI/CD pipeline: triggers on push to main, manual dispatch, and daily schedule (UTC 03:00). Uses a `check-version` job to detect the latest PicoClaw release and skip if already built, then a matrix `build` job producing both `picoclaw` and `picoclaw-browser` images for amd64/arm64.
